...
if __name__ == '__main__':
    ...
    with open(f'build/cache/{lang}.txt', 'r+') as cache:
        ...
        if CACHE:
            files = [f for f in files if not f.split('/')[-1].replace('.csv', '') in processed]
        ...
        with ProcessPoolExecutor(max_workers=20) as executor:
            ...
            for future in as_completed(futures):
                try:
                    # Apply timeout to each future's result method individually
                    result = future.result(timeout=10)  # Timeout for each individual task
